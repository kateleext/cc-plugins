...
## When to Use the Memory Briefer

Invoke the `memory-briefer` subagent when:

- **Exploratory** - not sure what session or even what terms to search
- **Multi-session** - topic likely spans multiple conversations
- **Reconstructing context** - "we discussed", "remember when", "why did we"
- **Before major decisions** - need full history before recommending

```
Task(subagent_type="memory-briefer", prompt="What's our auth implementation history?")
```

Use `deja` directly when:

- **Targeted** - you know the session or have specific search terms
- **Quick check** - just need to verify one thing
- **Following up** - briefer already gave you pointers, now reading specific turns

The briefer explores and synthesizes. The skill retrieves and verifies.
